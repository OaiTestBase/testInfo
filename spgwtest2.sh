...
hostname spgw
echo "public = ${public_floatingIp}" > ~/publicip.txt
SGiInterface=`ifconfig | grep -B1 "inet addr:${public_floatingIp}" | awk '$1!="inet" && $1!="--" {print $1}'`
S1_UInterface=`ifconfig | grep -B1 "inet addr:${public_floatingIp}" | awk '$1!="inet" && $1!="--" {print $1}'`
...
sudo sed -i "s/__SPGW_S1U_IP__/${public_floatingIp}/" /opt/openbaton/scripts/spgw.conf
