...
S1Interface=`ifconfig | grep -B1 "inet addr:${public_floatingIp}" | awk '$1!="inet" && $1!="--" {print $1}'`
S11Interface=`ifconfig | grep -B1 "inet addr:${S11}" | awk '$1!="inet" && $1!="--" {print $1}'`

#mod mme.conf
#S1 ip on mme
sudo sed -i "s/__MME_S1_C_IP__/${public_floatingIp}/" /opt/openbaton/scripts/mme.conf
#S1 interface on mme
